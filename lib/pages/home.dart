      ...
      appBar: AppBar (
        ...
        actions: <Widget>[ //앱바에서 아이콘을 왼쪽에 넣을 때는 actions위젯 오른쪽에 넣을 때는 leading사용
          IconButton( //홈 화면에서만 저장 버튼을 보여준다.
            ...
            onPressed: () {
              print("save button"+ mapArray[idx]["content"].toString());
              //금일 날짜의 같은 문장을 이미 저장했다면 저장안되게 수정해야함
              reference!.child(mapArray[idx]["writeUser"].toString())
                        .push()
                        .set(Write(mapArray[idx]["writeUser"].toString(), mapArray[idx]["content"].toString(), DateTime.now().toIso8601String())
                        .toJson())
                  .then((_) {
                Message();
              });
        ...
        child: Column(
          children: <Widget>[
            ...
            Container( //책 내용
              ...
              child: Text(mapArray[idx]["content"],
